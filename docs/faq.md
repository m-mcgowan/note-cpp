...
No. Sink mode parses responses with a SAX pipeline directly into your typed `Response` (or your own struct via `.into(T&)`) — no JSON library linked, no tree built. You only need a `JsonBackend` (cJSON, nlohmann, or `BufferJsonBackend`) when you want `response.body()` to return a walkable tree; see [`json-backend.md`](json-backend.md) for the trade-off.
