...
///
///   JsonTransact      — polymorphic, inherits the wide `ITransact`. Used
///                       by the polymorphic Notecard / BareNotecard paths.
///
///   JsonTransactT<ByteT> — templated, no vtable. Used by the static / AVR
///                          path (StaticNotecard); methods inline through
///                          the byte transport's methods so the compiler
///                          can fold both layers.
///
/// The polymorphic `JsonTransact` carries the full ITransact surface so it
/// can stand in for `Protocol` at the SerialTransportStack level. The
/// templated `JsonTransactT` carries the slimmer subset StaticNotecard
...
/// CRC-trailer writer shared by `JsonTransact` and `JsonTransactT`.
...
// ---------------------------------------------------------------------------
// JsonTransactT<ByteT> — templated, no-vtable JSON transact
// ---------------------------------------------------------------------------

/// Templated JsonTransact wrapping any IByteTransport-shaped type. No
...
// ---------------------------------------------------------------------------
// JsonTransact — polymorphic, inherits the wide `ITransact`
// ---------------------------------------------------------------------------
