...
struct HubLog {
    ...
    struct alert_t : Field<bool> {
        ...
        /// `true` if the message is urgent. This doesn't change any
        /// functionality, but rather `alert` is provided as a convenient flag
        /// to use in your program logic.
    ...
    struct sync_t : Field<bool> {
        using Field<bool>::Field;
        using Field<bool>::operator=;
        /// `true` if a sync should be initiated immediately. Setting `true`
...
#if NOTE_SINGLETON
inline ApiResult<void> HubLog::execute() const {
...
inline Result<void> HubLog::command() const {
    auto build_ = [&](JsonBuilder& b_) {
        b_.add("cmd", notecard_request);
        this->build(b_);
    };
