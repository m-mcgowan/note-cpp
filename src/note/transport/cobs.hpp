...
/// Use for buffer sizing. O(1).
inline constexpr size_t cobs_encoded_size(size_t len) {
    return len + (len / 254) + 1;
}

/// Exact encoded size for specific data (excluding the EOP byte).
/// Scans for zero bytes which affect COBS block boundaries. O(n), uses memchr.
/// Use this for the `cobs` field in the JSON handshake.
inline size_t cobs_encoded_length(const uint8_t* data, size_t len) {
    size_t encoded = 1;  // initial code byte
    uint8_t code = 1;
    while (len > 0) {
        size_t max_run = 0xFF - code;
        size_t search = len < max_run ? len : max_run;
        auto* zero = static_cast<const uint8_t*>(memchr(data, 0, search));
        size_t chunk = zero ? static_cast<size_t>(zero - data) : search;
        encoded += chunk;
        data += chunk;
        len -= chunk;
        code += static_cast<uint8_t>(chunk);
        if (zero && len > 0) {
            encoded++;  // new code byte after zero
            code = 1;
            data++;     // skip the zero
            len--;
        } else if (code == 0xFF) {
            encoded++;  // new code byte at block boundary
            code = 1;
        }
    }
    return encoded;
