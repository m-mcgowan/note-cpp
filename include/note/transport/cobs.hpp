...
/// encoded blocks to a callable. No allocation, bounded stack usage (255 bytes).
///
/// @code
...
public:
    ...
    /// Encode `src` (len bytes) and flush encoded blocks to `flush`.
    /// Source is read-only. Each flush call emits at most 255 bytes.
...
/// Streaming COBS decoder. Feeds encoded bytes in and emits decoded output
/// to a callable. Maintains state between feed() calls, so encoded data can
/// arrive in any chunk size — from one byte at a time to the entire payload.
///
/// @code
/// note::CobsDecoder decoder;
/// decoder.reset();
/// // As bytes arrive from the transport:
...
class CobsDecoder {
public:
    explicit CobsDecoder(uint8_t eop = cobs_eop) : eop_(eop) { reset(); }

    /// Reset decoder state for a new message.
    void reset() {
        remaining_ = 0;
        first_ = true;
        no_zero_ = false;
    ...
    /// Feed encoded bytes. Decoded output is emitted to `out` as it becomes
    /// available. Returns false if a termination marker (code == 0) was seen.
    ...
    bool feed(const uint8_t* data, size_t len, Out&& out) {
        ...
        return true;  // more data expected
    }

    /// Flush any buffered decoded bytes to `out`.
    template<typename Out>
    void flush(Out&& out) {
        if (buf_len_ > 0) {
            out(buf_, buf_len_);
