...
- **Zero-heap operation** — pair the library with a `MonotonicArena` (or `HeapResetPool`) and the entire send/receive cycle stays out of `malloc`. The same typed API works on an Arduino Uno with no heap at all, the [arena lifecycle](docs/memory.md#picking-an-allocator) reclaims memory by `reset()` rather than per-allocation.
