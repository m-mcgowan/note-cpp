...
// ---------------------------------------------------------------------------
TEST_CASE("scan::field finds flat string value") {
    auto v = note::scan::field(R"({"name":"abc","x":1})", "name");
    REQUIRE(v == R"("abc")");
}

TEST_CASE("scan::field finds flat number value") {
    auto v = note::scan::field(R"({"x":42,"y":"z"})", "x");
    REQUIRE(v == "42");
}

TEST_CASE("scan::field stops at trailing comma") {
    auto v = note::scan::field(R"({"a":1,"b":2})", "a");
    REQUIRE(v == "1");
}

TEST_CASE("scan::field stops at closing brace") {
    auto v = note::scan::field(R"({"only":7})", "only");
    REQUIRE(v == "7");
}

TEST_CASE("scan::field returns empty for missing key") {
    auto v = note::scan::field(R"({"x":1})", "missing");
    REQUIRE(v.empty());
}

TEST_CASE("scan::field skips whitespace after colon") {
    auto v = note::scan::field(R"({"x":   123})", "x");
    REQUIRE(v == "123");
}

TEST_CASE("scan::field tolerates whitespace around key") {
    auto v = note::scan::field(R"({ "x" : 5 })", "x");
    REQUIRE(v == "5");
}

TEST_CASE("scan::field returns object substring for object value") {
    auto v = note::scan::field(R"({"body":{"a":1}})", "body");
    REQUIRE(v == R"({"a":1})");
}

TEST_CASE("scan::field returns array substring for array value") {
...
// ---------------------------------------------------------------------------
TEST_CASE("scan::object returns flat object") {
    auto v = note::scan::object(R"({"body":{"temp":21.5}})", "body");
    REQUIRE(v == R"({"temp":21.5})");
}

TEST_CASE("scan::object handles nested braces") {
    auto v = note::scan::object(R"({"body":{"inner":{"x":1},"y":2}})", "body");
    REQUIRE(v == R"({"inner":{"x":1},"y":2})");
}

TEST_CASE("scan::object returns empty when key missing") {
    auto v = note::scan::object(R"({"body":{"x":1}})", "missing");
    REQUIRE(v.empty());
}

TEST_CASE("scan::object returns empty when value is not an object") {
...
// ---------------------------------------------------------------------------
TEST_CASE("scan::array returns flat array") {
    auto v = note::scan::array(R"({"xs":[1,2,3]})", "xs");
    REQUIRE(v == "[1,2,3]");
}

TEST_CASE("scan::array handles nested arrays") {
    auto v = note::scan::array(R"({"xs":[[1,2],[3,4]]})", "xs");
    REQUIRE(v == "[[1,2],[3,4]]");
}

TEST_CASE("scan::array returns empty when key missing") {
    auto v = note::scan::array(R"({"xs":[1]})", "ys");
    REQUIRE(v.empty());
}

TEST_CASE("scan::array returns empty when value is not an array") {
...
// ---------------------------------------------------------------------------
TEST_CASE("scan::get<int> extracts integer") {
    auto v = note::scan::get(R"({"n":42})", "n", int32_t{0});
    REQUIRE(v == 42);
}

TEST_CASE("scan::get<int> returns default for missing key") {
    auto v = note::scan::get(R"({"x":1})", "n", int32_t{7});
    REQUIRE(v == 7);
}

TEST_CASE("scan::get<double> extracts decimal") {
    auto v = note::scan::get(R"({"t":21.5})", "t", 0.0);
    REQUIRE(v == doctest::Approx(21.5));
}

TEST_CASE("scan::get<float> extracts float") {
    auto v = note::scan::get(R"({"t":3.25})", "t", 0.0f);
    REQUIRE(v == doctest::Approx(3.25f));
}

TEST_CASE("scan::get<bool> extracts true") {
    auto v = note::scan::get(R"({"on":true})", "on", false);
    REQUIRE(v == true);
}

TEST_CASE("scan::get<bool> extracts false") {
    auto v = note::scan::get(R"({"on":false})", "on", true);
    REQUIRE(v == false);
}

TEST_CASE("scan::get<bool> returns default for missing") {
    auto v = note::scan::get(R"({"x":1})", "on", true);
    REQUIRE(v == true);
}

TEST_CASE("scan::get<string_view> returns value without quotes") {
    auto v = note::scan::get(R"({"name":"abc"})", "name", string_view{"?"});
    REQUIRE(v == "abc");
}

TEST_CASE("scan::get<string_view> returns default for missing") {
...
// ---------------------------------------------------------------------------
TEST_CASE("nested body extraction") {
...
TEST_CASE("JsonView returns empty view for missing object") {
...
TEST_CASE("JsonView array accessor") {
...
// ---------------------------------------------------------------------------
TEST_CASE("scan::for_each visits all top-level pairs") {
    std::string keys;
    std::string values;
...
TEST_CASE("scan::for_each skips nested objects as single values") {
...
TEST_CASE("scan::for_each handles whitespace") {
    int n = 0;
...
TEST_CASE("scan::for_each on empty object") {
    int n = 0;
    note::scan::for_each("{}",
        [&](note::string_view, note::string_view) { ++n; });
    REQUIRE(n == 0);
...
TEST_CASE("scan::for_each stops at matching brace") {
...
TEST_CASE("scan::into populates all fields") {
...
TEST_CASE("scan::into ignores unknown keys") {
    Readings r{0.0f, 0, false};
...
TEST_CASE("scan::into leaves missing fields untouched") {
...
TEST_CASE("JsonView::into forwards to scan::into") {
...
TEST_CASE("scan::into with pick tag matches walk result") {
...
TEST_CASE("JsonView::into with pick tag") {
...
TEST_CASE("scan tag via using alias") {
...
TEST_CASE("JsonView constructs from std::optional<string_view>") {
...
TEST_CASE("scan::field accepts FlashString key") {
    auto v = note::scan::field(R"({"temperature":21.5,"humidity":60})",
                               note::flash(k_temp));
    REQUIRE(v == "21.5");
}

TEST_CASE("scan::object accepts FlashString key") {
    auto v = note::scan::object(R"({"body":{"a":1}})", note::flash(k_body));
    REQUIRE(v == R"({"a":1})");
}

TEST_CASE("scan::get_float accepts FlashString key") {
    auto v = note::scan::get_float(R"({"temperature":4.2})",
                                   note::flash(k_temp));
    REQUIRE(v == doctest::Approx(4.2f));
}

TEST_CASE("scan::get_int accepts FlashString key + default") {
...
TEST_CASE("JsonView chains with flash keys") {
...
TEST_CASE("JsonView::for_each visits pairs") {
