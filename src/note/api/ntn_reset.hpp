...
template<>
struct request_traits<::note::api::NtnReset> {
#if NOTE_SINGLETON
...
#if NOTE_SINGLETON
inline ApiResult<void> NtnReset::execute() const {
    ...
    using meta_ = ::note::detail::request_traits<NtnReset>;
...
inline Result<void> NtnReset::command() const {
    auto build_ = [&](JsonBuilder& b_) {
        b_.add("cmd", notecard_request);
        this->build(b_);
    };
