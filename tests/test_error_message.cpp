...
// ---------------------------------------------------------------------------
TEST_CASE("ErrorMessage defaults to empty") {
    ErrorMessage m;
    REQUIRE(m.size() == 0);
    REQUIRE(m.empty());
...
TEST_CASE("ErrorMessage from string_view") {
...
TEST_CASE("ErrorMessage from const char*") {
    const char* s = "hello";
    ErrorMessage m{s};
    REQUIRE(m.size() == 5);
...
TEST_CASE("ErrorMessage from FlashString") {
...
TEST_CASE("ErrorMessage comparison rejects wrong length") {
...
TEST_CASE("ErrorMessage comparison rejects mismatched content") {
    ErrorMessage m{note::flash(k_err)};
...
// ---------------------------------------------------------------------------
TEST_CASE("ErrorInfo constructs from RAM literal") {
...
TEST_CASE("ErrorInfo constructs from FlashString") {
...
TEST_CASE("to_string(ErrorInfo) handles flash-backed message") {
    note::ErrorInfo e{note::Error::Overflow, note::flash(k_err)};
